...
class FirebaseStorageManagerPost {
    ...
    fun uploadImage(mContext: Context, imageURI: Uri) {
        ...
        val uploadTask = mStorageRef.child(user).child("post").putFile(imageURI)
        uploadTask.addOnSuccessListener {
            Log.e(TAG, "upload success")
            val downloadURLTask = mStorageRef.child(user).child("post").downloadUrl
